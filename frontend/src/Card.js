...
const generateNumbers = (luckyNumber) => {
  ...
  return Array.from({ length: 18 })
    ...
    .map((_, i) => {
      ...
      if (
        num !== luckyNumber ||
        (num === luckyNumber && luckyNumber === random)
      ) {
        return num;
      }
      return arr.pop() || 0;
...
const prize = [
  5, 10, 15, 20, 30, 40, 50, 100, 150, 200, 250, 300, 400, 500, 600, 700, 800,
  999,
];
...
function Card() {
  ...
  const prizeIndex = useRef(
    getPrizeIndex(luckyNumber.current, numbers.current)
  );
  ...
  const card = useMemo(() => {
    ...
    image.onload = () => {
      ...
      numbers.current.forEach((num, i) => {
        ...
        const y = 312 + Math.trunc(i / 6) * 48;
        ctx0.fillStyle =
          num === luckyNumber.current || num === today ? "#ffcc16" : "white";
        ctx0.font = "16px sans";
        ...
        ctx0.fillText(
          `￥${prize[i]}`,
          x - ctx0.measureText(`￥${prize[i]}`).width / 4,
          y + 16
        );
      });
      prize.forEach((price, index) => {
  ...
  useEffect(() => {
    const animate = () => {
      ...
      const targetRotationX =
        (Math.PI / 16) * Math.max(-1, Math.min(1, mouse.x));
      const targetRotationY =
        (Math.PI / 36) * Math.max(-1, Math.min(1, mouse.y));
  ...
  useEffect(() => {
    ...
    const touchHandler = (event) => {
      setMouse({
        ...
        y:
          (event.changedTouches[0].clientY / (window.innerHeight * 0.8)) * 2 -
          1,
      });
    };
  ...
  useEffect(() => {
    ...
    const touchMoveHandler = (event) => {
      ...
      pointer.current.y =
        (event.changedTouches[0].clientY / (window.innerHeight * 0.8)) * 2 - 1;
    };
